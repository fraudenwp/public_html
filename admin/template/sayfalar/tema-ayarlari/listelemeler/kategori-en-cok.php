...
function getCellContent($key, $value, $id, $tablo_adi, $sutun_adi, $satir_adi, $user_dil, $pdo) {
    switch ($key) {
        case 'Sıra_No':
        ...
        case 'Başlık':
			...
			case 'Kategori':
            // Kategori ID'sini al
            $kategori_id = is_array($value) ? ($value[$user_dil] ?? $value['tr'] ?? reset($value)) : $value;
            ...
            $stmt = $pdo->prepare("
                WITH ranked_categories AS (
                    SELECT 
                        ...
                        ROW_NUMBER() OVER (
                            PARTITION BY grup 
                            ORDER BY CASE WHEN dil = :user_dil THEN 1 ELSE 2 END
                        ) as row_num
                    FROM kategoriler 
                    WHERE yayin_durumu = 1 AND grup = :grup
                )
                SELECT baslik
                FROM ranked_categories 
                WHERE row_num = 1
            ");
            $stmt->execute(['user_dil' => $user_dil, 'grup' => $kategori_id]);
            $kategori = $stmt->fetch(PDO::FETCH_ASSOC);
            
            return "<td><a href='#' data-toggle='modal' data-formturu='veri-guncelle' data-tabloadi='{$tablo_adi}' data-sutun-adi='{$sutun_adi}' data-satir-adi='{$satir_adi}' data-target='#GuncelleModalForm' data-modalduzenleid='" . htmlspecialchars($id) . "'>" . htmlspecialchars($kategori['baslik'] ?? 'Kategori Bulunamadı') . "</a></td>";
...
/// Dile göre kategorileri çek ilgili dil yoksa aynu gruptaki farklı bir kategoriyi çek
$query = $pdo->prepare("
    WITH ranked_categories AS (
        SELECT 
            id, yayin_durumu, baslik, grup, dil,
            ROW_NUMBER() OVER (
                PARTITION BY grup 
                ORDER BY CASE WHEN dil = :user_dil THEN 1 ELSE 2 END
            ) as row_num
...
<div class="card card-primary card-outline">
    <div class="card-header">
        <h3 class="card-title">#<?php echo $veri_id; ?> Tek Kategori Ürün Listele </h3>
				...
				<?php foreach ($veri_data as $veri): ?>
					<tr id="tr<?php echo htmlspecialchars($veri['id']); ?>">
						...
						<?php foreach ($icerik_cek_array as $key): ?>
							<?php echo getCellContent($key, $veri[$key] ?? '', $veri['id'], $tablo_adi, $sutun_adi, $satir_adi, $user_dil, $pdo); ?>
						<?php endforeach; ?>
					</tr>
				<?php endforeach; ?>
				</tbody>
    ...
    <div class="modal-content">
      <div class="modal-header">
		...
		<div class="modal-body">
		  <form id="ekleForm" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<select class="form-control select2bs4" style="width: 100%;" name="kategori" data-name="Kategori">
					<option value="">Seç</option>
					<?php foreach ($kategoriler as $kategori): ?>
						<option value="<?php echo htmlspecialchars($kategori['grup']); ?>">
							<?php echo htmlspecialchars($kategori['baslik']); ?>
						</option>
					<?php endforeach; ?>
				</select>
			</div>			
			...
			<div class="form-group">
			  ...
			  <input type="text" class="form-control" data-name="Limit" name="limit" value="10">
			</div>
		...
		<div class="modal-footer">
		  ...
		  <button type="button" id="KaydetButon" data-formturu="veri-ekle" data-tabloadi="tema" data-sutun-adi="tur" data-satir-adi="kategori-en-cok" class="btn btn-primary">Kaydet</button>
		</div>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" id="SilButon" data-formturu="veri-sil" data-tabloadi="tema" data-sutun-adi="tur" data-satir-adi="kategori-en-cok"  class="btn btn-primary">Evet</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="GuncelleModalForm" tabindex="-1" role="dialog" aria-labelledby="GuncelleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="GuncelleModalLabel">Güncelle <?php echo $gizle; ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
		<div class="modal-body">
		<img id="modal-resim" src="" alt="Resim" style="margin-bottom: 10px; height: 100px; <?php if( $gizle == 1 ) { echo 'display:none;'; } ?>">
		  <form id="guncelleForm" enctype="multipart/form-data">
			...
			<div class="form-group">
			  <label for="Limit">Limit</label>
			  <input type="text" class="form-control" data-name="Limit" name="limit">
			</div>			
			<div class="form-group" style="<?php if( $gizle == 1 ) { echo 'display:none;'; } ?>">
			  <label for="resim">Resim</label>
			  <input type="file" data-resim_en="350" data-name="Resim" data-resim_boy="200" data-resim_doldur="false" data-resim_kirp="true" data-resim_turu="png" data-resim_yolu="../../resimler/slide/" class="form-control" name="resim">
		...
		<div class="modal-footer">
		  <button type="button" class="btn btn-secondary" data-dismiss="modal">Kapat</button>
		  <button type="button" id="GuncelleButon" data-formturu="veri-guncelle" data-tabloadi="tema" data-sutun-adi="tur" data-satir-adi="kategori-en-cok" class="btn btn-primary">Güncelle</button>
		</div>
    </div>
  </div>
</div>
