...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM bilgi_sayfalari WHERE id = ?");
    ...
    $current_table = isset($_GET['tablo']) ? $_GET['tablo'] : 'bilgi_sayfalari';
    ...
    $tam_url = 'https://' . $_SERVER['HTTP_HOST'] . '/bilgi-sayfalari-detay/' . $sayfaData_id . '/' . $sayfaData_link;
...
<?php endif; ?>
         <h1 <?= $baslik_gizle === "1" ? 'style="display:none;"' : '' ?>>
...
<!-- /SEO Internal Link -->
	...
	<?php if($iletisim_bilgileri_gizle == 1){ ?>
		...
		<div class="list-group">
			  ...
			  <a href="https://wa.me/<?php echo formatWhatsAppNumber($kurumsalData['contact']['whatsapp']); ?>" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
